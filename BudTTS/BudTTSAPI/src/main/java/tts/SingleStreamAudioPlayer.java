...
public class SingleStreamAudioPlayer implements AudioPlayer {
	...
	public SingleStreamAudioPlayer(String baseName, AudioFileFormat.Type type) {
	...
	public SingleStreamAudioPlayer() {
...
//				LOGGER.info("Avail " + ais.available());
//				LOGGER.info("totBytes " + totBytes);
//				LOGGER.info("FS " + currentFormat.getFrameSize());
//			//}
//			LOGGER.info("Wrote synthesized speech to " + baseName);
			ByteArrayOutputStream os = new ByteArrayOutputStream(totBytes);
			AudioSystem.write(ais, outputType, os);
			final byte[] contents = os.toByteArray();
	        this.audioOutput = new AudioOutput(baseName, outputType.toString(), is, totBytes,contents);
			is.close();
			ais.close();
